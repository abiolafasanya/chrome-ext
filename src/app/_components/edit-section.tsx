...
const EditSection = () => {
  ...
  const playBack = useSearchParams().get('recording');
  const [filename, setFilename] = useState<string>(() =>
    searchParams ? searchParams : ''
  );
  async function copyVideo(url: string) {
    ...
    <article className='w-full lg:w-1/2'>
      ...
      <div className='border-2 py-3 px-5 rounded-lg flex justify-between mb-10'>
        ...
        >
          ...
          <span onClick={() => copyVideo(playBack ? playBack : '')}>Copy</span>
        </Button>
      </div>
      ...
      <div className='py-3 rounded-lg flex gap-5 mb-10'>
        {socials.map((social) => (
          <Fragment key={social.id}>
            <Link
              href={social.link}
              className='flex gap-2 rounded-lg border-2 py-3 px-5'
            >
